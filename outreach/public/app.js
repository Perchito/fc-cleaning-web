// Strip ?token= from the address bar once the cookie is set by the server.
...
let STATE = { prospects: [], counts: {}, config: {} };
...
function render() {
  ...
  $("#sub").textContent =
    `${total} prospect${total === 1 ? "" : "s"} · from ${STATE.config.from || ""}`;

  const order = ["replied", "follow_up_due", "awaiting_reply", "draft", "bounced"];
  $("#chips").replaceChildren(
    ...order
      .filter((s) => counts[s])
      .map((s) => el("span", { className: "chip" }, [
  ...
  list.replaceChildren(...prospects.map(card));
}
...
function card(p) {
  ...
  if (p.lastSendAt)
    bits.push(
      `${p.lastSendType === "initial" ? "First email" : "Follow-up"} ${timeAgo(p.lastSendAt)}`,
    );
  ...
  if (p.autoAckAt && p.effectiveStatus !== "replied" && p.effectiveStatus !== "bounced") {
    c.append(
      el("div", { className: "meta", textContent: `✓ Auto-acknowledgement received ${timeAgo(p.autoAckAt)} — email definitely landed` }),
    );
  }
  ...
  if (p.effectiveStatus === "draft") {
    actions.append(btn("Send first email", "primary", () => openDraft(p)));
  } else if (p.effectiveStatus === "follow_up_due") {
    actions.append(btn("Send follow-up", "primary", () => openDraft(p)));
  } else if (p.effectiveStatus === "awaiting_reply") {
    actions.append(btn("Send follow-up now", "", () => openDraft(p)));
  } else if (p.effectiveStatus === "replied") {
    actions.append(btn("Send follow-up", "", () => openDraft(p)));
  }
  ...
  sel.onchange = async () => {
    if (!sel.value) return;
    try {
      await api(`/api/prospects/${p.id}/status`, {
        method: "POST",
...
async function openDraft(p) {
  ...
  dlg.showModal();
  try {
    const d = await api(`/api/prospects/${p.id}/draft`);
    $("#d-title").textContent =
...
$("#d-send").onclick = async () => {
  ...
  try {
    const res = await api(`/api/prospects/${DRAFT_FOR.id}/send`, {
      method: "POST",
      body: JSON.stringify({
